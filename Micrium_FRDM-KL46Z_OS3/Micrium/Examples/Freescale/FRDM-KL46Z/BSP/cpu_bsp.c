...
void  CPU_TS_TmrInit (void)
{
    CPU_INT32U  cpu_clk_freq_hz;
                                                                ...
                                                                /* The Cycle counter is not supported in the Cortex-M0  */
                                                                /* so instead will use the TPM1 module.                 */
#if 0
    SIM_SCGC6   |= SIM_SCGC6_TPM1_MASK;                         /* Enable the TPM1 clocks.                              */
    ...
    TPM1_C0V     = 0;                                           /* Initial value.                                       */

    TPM1_SC      = TPM_SC_CMOD(1) | TPM_SC_PS(0);               /* Up counter free running from BUSCLK / 1.             */
#endif

    cpu_clk_freq_hz = BSP_CPU_ClkFreq();
    CPU_TS_TmrFreqSet(cpu_clk_freq_hz * 5);
}
#endif
...
#if 0
    ts_tmr_cnts = (CPU_TS_TMR)TPM1_CNT;
#else
    ts_tmr_cnts = 0;
#endif
